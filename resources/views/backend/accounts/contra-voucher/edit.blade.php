    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form id="contraVoucherForm" method="POST" action="{{ route('contra-voucher.update', $contraVoucher->id) }}">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="trans_date" name="trans_date" 
                                           value="{{ old('trans_date', $contraVoucher->trans_date ? $contraVoucher->trans_date->format('Y-m-d') : date('Y-m-d')) }}" required>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="total_amount" name="total_amount" 
                                           step="0.01" value="{{ old('total_amount', $contraVoucher->amount) }}" readonly>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="remarks" name="remarks" 
                                           value="{{ old('remarks', $contraVoucher->comments) }}" placeholder="Enter remarks">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('from_ledger') is-invalid @enderror" id="from_ledger" name="from_ledger">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('to_ledger') is-invalid @enderror" id="to_ledger" name="to_ledger">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('transfer_amount') is-invalid @enderror" 
                                           id="transfer_amount" name="transfer_amount" step="0.01" min="0" placeholder="0.00">
                                        ...
                                        <tbody id="lineItemsBody">
                                            @if($contraVoucher->accTransactionDetails && $contraVoucher->accTransactionDetails->count() > 0)
                                                @foreach($contraVoucher->accTransactionDetails as $detail)
                                                    @php
                                                        $fromLedger = \App\Http\Controllers\Account\Models\SubsidiaryLedger::find($detail->dr_sub_ledger);
                                                        $toLedger = \App\Http\Controllers\Account\Models\SubsidiaryLedger::find($detail->cr_sub_ledger);
                                                    @endphp
                                                    <tr>
                                                        <td>
                                                            <select class="form-control select2 @error('group_id') is-invalid @enderror general_ledger" name="line_items[{{ $loop->index }}][from_ledger_id]" required>
                                                                <option value="">-- Choose From Ledger --</option>
                                                                @foreach($groups as $group)
                                                                    <optgroup label="{{ $group->name }} ({{ $group->code }})">
                                                                        @foreach($group->subsidiaryLedgers as $subsidiary)
                                                                            <option value="{{ $subsidiary->id }}" {{ $detail->dr_sub_ledger == $subsidiary->id ? 'selected' : '' }} data-group="{{ $group->name }}">
                                                                                {{ $subsidiary->name }} ({{ $subsidiary->ledger_code }})
                                                                            </option>
                                                                        @endforeach
                                                                    </optgroup>
                                                        ...
                                                        <td>
                                                            <select class="form-control select2 @error('to_ledger') is-invalid @enderror to_ledger" name="line_items[{{ $loop->index }}][to_ledger_id]" required>
                                                                <option value="">-- Choose To Ledger --</option>
                                                                @foreach($groups as $group)
                                                                    <optgroup label="{{ $group->name }} ({{ $group->code }})">
                                                                        @foreach($group->subsidiaryLedgers as $subsidiary)
                                                                            <option value="{{ $subsidiary->id }}" {{ $detail->cr_sub_ledger == $subsidiary->id ? 'selected' : '' }} data-group="{{ $group->name }}">
                                                                                {{ $subsidiary->name }} ({{ $subsidiary->ledger_code }})
                                                                            </option>
                                                                        @endforeach
                                                                    </optgroup>
...
$(document).ready(function() {
    ...
    $('#contraVoucherForm').on('submit', function(e) {
        ...
        lineItems.forEach((item, index) => {
            $(this).append(`<input type="hidden" name="line_items[${index}][from_ledger_id]" value="${item.fromLedger}">`);
            $(this).append(`<input type="hidden" name="line_items[${index}][to_ledger_id]" value="${item.toLedger}">`);
            $(this).append(`<input type="hidden" name="line_items[${index}][amount]" value="${item.transferAmount}">`);
        });
